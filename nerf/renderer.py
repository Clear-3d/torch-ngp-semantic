...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        sem = self.semantic(xyzs.reshape(-1, 3), **density_outputs)
        sem = sem.view(N, -1, gl.get_value('num_class'))
        ...
        image_sem = torch.sum(weights.unsqueeze(-1) * sem, dim=-2) 
        ...
        image_sem = image_sem.view(*prefix, gl.get_value('num_class'))
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.training:
            ...
            sigmas, rgbs, rgbs_sem = self(xyzs, dirs)# 1 3 num_class
            ...
            if len(sigmas.shape) == 2:
                ...
                weights_sum_sem, depth_sem, image_sem = raymarching.composite_rays_train(sigmas, rgbs_sem, deltas, rays, T_thresh)
                image_sem = image_sem
                depth_sem = depth
                image_sem = image_sem.view(*prefix, gl.get_value('num_class'))
                depth_sem = depth_sem.view(*prefix)
            ...
            weights_sum_sem = torch.zeros(N, dtype=dtype, device=device)
            depth = torch.zeros(N, dtype=dtype, device=device)
            depth_sem = torch.zeros(N, dtype=dtype, device=device)
            image = torch.zeros(N, 3, dtype=dtype, device=device)
            image_sem = torch.zeros(N,dtype=torch.uint8,device=device)
            ...
            while step < max_steps:
                ...
                sigmas, rgbs, rgbs_sem = self(xyzs, dirs)
                # density_outputs = self.density(xyzs) # [M,], use a dict since it may include extra things, like geo_feat for rgb.
                # sigmas = density_outputs['sigma']
                # rgbs = self.color(xyzs, dirs, **density_outputs)
                sigmas = self.density_scale * sigmas
                ...
                raymarching.composite_rays(n_alive, n_step, rays_alive, rays_t, sigmas, rgbs_sem, deltas, weights_sum_sem, depth_sem, image_sem, T_thresh)
            ...
            image = image.view(*prefix, 3)
            depth = depth.view(*prefix)
            image_sem = image_sem(*prefix, gl.get_value('num_class'))
            depth_sem = depth
        
        results['depth'] = depth
        results['image'] = image
        
        results['depth_sem']=depth_sem
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096*5, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3] pred_rgb_sem [B, N, num_class]
        ...
        if staged and not self.cuda_ray:
            ...
            image_sem = torch.empty((B,N,gl.get_value('num_class')),device=device)
            for b in range(B):
                head = 0
                while head < N:
