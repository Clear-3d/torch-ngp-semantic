...
def Iou(input,target,classNum):
    ...
    class_real = len(np.unique(target))
    batchMious[0] = batchMious[0]*classNum/class_real
    return batchMious
...
class MIOUMeter:
    ...
    def  __init__(self, res=0, device=None):
        self.res=res

    def write(self, writer, global_step, prefix=""):
        writer.add_scalar(os.path.join(prefix, "MIOU"), self.res, global_step)

    def report(self):
        return f'MIOU = {self.res:.6f}'
    
    def measure(input,target,classNum):  #Iou
...
class Trainer(object):
    def __init__(self, 
        ...
        if isinstance(criterion[0], nn.Module):
            criterion[0].to(self.device)
        if isinstance(criterion[1], nn.Module):
            criterion[1].to(self.device)
    ...
    def train_step(self, data):
        ...
        pred_rgb_sem = outputs['image_sem']
        # MSE loss
        loss1 = self.criterion(pred_rgb, gt_rgb).mean(-1) # [B, N, 3] --> [B, N]
        
        temp = gt_rgb_sem[0][:, 0].long()
        class_count = torch.bincount(temp, minlength=gl.get_value('num_class')).float()
        tmax = torch.max(class_count)
        weights = np.log2(tmax.cpu() / class_count.cpu())+1
        
        class_weights = weights.to(self.device)
        class_weights[class_weights == float('inf')] = 1
        loss_fn = torch.nn.CrossEntropyLoss(weight=class_weights)
        loss2 = loss_fn(pred_rgb_sem[0], gt_rgb_sem[0][:,0].long()).mean(-1)        

        if gl.get_value('loss_ce')==0:
            loss=loss1.mean()
        else:
            loss=loss1.mean()+0.04*loss2 # come from semantic-nerf
        ...
        return pred_rgb, gt_rgb,pred_rgb_sem,gt_rgb_sem, loss
    ...
    def eval_step(self, data):
        ...
        pred_sem = outputs['image_sem'].reshape(B, H, W,gl.get_value('num_class'))
      #  pred_depth_sem = outputs['depth_sem'].reshape(B, H, W)

        loss = self.criterion(pred_rgb, gt_rgb).mean()

        temp = gt_sem[0].reshape(H*W).long()
        class_count = np.bincount(temp.cpu(), minlength=gl.get_value('num_class'))
        tmax = np.max(class_count)
        
        weights = np.log2(tmax / class_count)+1
        class_weights = torch.FloatTensor(weights).to(self.device)
        class_weights[class_weights == float('inf')] = 1
        loss_fn = torch.nn.CrossEntropyLoss(weight=class_weights)

        loss2 = loss_fn(pred_sem[0].reshape(H*W,gl.get_value('num_class')), gt_sem[0].reshape(H*W).long()).mean(-1)

        return pred_rgb, pred_depth,pred_sem, gt_rgb, gt_sem, loss, loss2
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        pred_sem = outputs['image_sem'].reshape(-1, H, W,gl.get_value('num_class'))
    ...
    def test(self, loader, save_path=None, name=None, write_video=True):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                pred_sem = preds_sem[0].detach().cpu().numpy()
                pred_sem_res = np.argmax(pred_sem,axis=2)

                seg = pred_sem_res
                color_seg_pre = np.zeros((seg.shape[0], seg.shape[1], 3), dtype=np.uint8)   
                for label, color in enumerate(PALETTE):
                    color_seg_pre[seg == label, :] = color
                    color_seg_pre = color_seg_pre[..., ::-1] # convert to BGR

                
                if write_video:
                    all_preds.append(pred)
                    all_preds_depth.append(pred_depth)
                    all_preds_sem.append(color_seg_pre)
                else:
                    cv2.imwrite(os.path.join(save_path, f'{name}_{i:04d}_rgb.png'), cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(os.path.join(save_path, f'{name}_{i:04d}_depth.png'), pred_depth)
                    cv2.imwrite(os.path.join(save_path, f'{name}_{i:04d}_semantic.png'), cv2.cvtColor(color_seg_pre, cv2.COLOR_RGB2BGR))
        ...
        if write_video:
            ...
            all_preds_sem = np.stack(all_preds_sem, axis=0)
            imageio.mimwrite(os.path.join(save_path, f'{name}_rgb.mp4'), all_preds, fps=25, quality=8, macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_depth.mp4'), all_preds_depth, fps=25, quality=8, macro_block_size=1)
            imageio.mimwrite(os.path.join(save_path, f'{name}_semantic.mp4'), all_preds_sem, fps=25, quality=8, macro_block_size=1)
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                preds, truths,preds_sem,truths_sem, loss = self.train_step(data)  #pred_rgb, gt_rgb,pred_rgb_sem,gt_rgb_sem, loss
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1):
        ...
        for label, color in enumerate(PALETTE):
            color_seg_pre[seg == label, :] = color
            color_seg_pre = color_seg_pre[..., ::-1] # convert to BGR
        color_seg_pre = (color_seg_pre / 255.0).astype(np.float32)
        outputs = {
            'image': pred,
            'depth': pred_depth,
            'image_sem':color_seg_pre
        }
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                preds, truths, preds_sem, truths_sem, loss = self.train_step(data)
            ...
            if self.local_rank == 0:
                ...
                if self.scheduler_update_every_step:
                    pbar.set_description(f"loss={loss_val:.4f} , lr={self.optimizer.param_groups[0]['lr']:.6f}")#({total_loss/self.local_step:.4f})
                else:
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth,preds_sem, truths,truths_sem, loss,loss2 = self.eval_step(data)
                ...
                if self.local_rank == 0:
                    ...
                    save_path_sem = os.path.join(self.workspace, 'validation', f'{name}_{self.local_step:04d}_sem.png')
                    save_path_sem_truth = os.path.join(self.workspace, 'validation', f'{name}_{self.local_step:04d}_sem_truth.png')
                    ...
                    pred_sem = preds_sem[0].detach().cpu().numpy()
                    pred_sem_truth = truths_sem[0].detach().cpu().numpy()
                    pred_sem_truth = pred_sem_truth.reshape(pred_sem_truth.shape[0],pred_sem_truth.shape[1])
                    pred_sem_res = np.argmax(pred_sem,axis=2)

                    miou=Iou(torch.tensor(pred_sem_res.reshape(1,pred_sem_res.shape[0],pred_sem_res.shape[1]).astype(int)),torch.tensor(pred_sem_truth.reshape(1,pred_sem_res.shape[0],pred_sem_res.shape[1]).astype(int)),gl.get_value('num_class'))
                    
                    # colour pred_sem_truth pred_sem_res
                    seg = pred_sem_truth
                    color_seg_truth = np.zeros((seg.shape[0], seg.shape[1], 3), dtype=np.uint8)   
                    for label, color in enumerate(PALETTE):
                        color_seg_truth[seg == label, :] = color
                        color_seg_truth = color_seg_truth[..., ::-1] # convert to BGR
                    seg = pred_sem_res
                    color_seg_pre = np.zeros((seg.shape[0], seg.shape[1], 3), dtype=np.uint8)   
                    for label, color in enumerate(PALETTE):
                        color_seg_pre[seg == label, :] = color
                    ...
                    cv2.imwrite(save_path_sem, cv2.cvtColor(color_seg_pre, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(save_path_sem_truth,cv2.cvtColor(color_seg_truth, cv2.COLOR_RGB2BGR))

                    pbar.set_description(f"mseloss={loss_val:.4f} CEloss={loss2_val:.4f} ({total_loss/self.local_step:.4f})")
                    pbar.update(loader.batch_size)
        ...
        if self.local_rank == 0:
            ...
            for metric in self.metrics:
                ...
                metric.clear()
            temp=MIOUMeter(res=miou[0])
            self.log(temp.report(),style="blue")
            if self.use_tensorboardX:
                temp.write(self.writer, self.epoch, prefix="evaluate")
        if self.ema is not None:
            self.ema.restore()
